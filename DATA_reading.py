...
def OIFITS_READING_concatenate(filename):
    ...
    with fits.open(filename, memmap=False) as fichier:
        ...
        for s in range(len(index_array)):
            STA_INDEX_list.append(fichier[index_array[s]].data['STA_INDEX'])
            TEL_NAME_list.append(fichier[index_array[s]].data['TEL_NAME'])
        
        STA_INDEX_tmp = np.vstack(STA_INDEX_list)
        TEL_NAME_tmp  = np.vstack(TEL_NAME_list)
        ...
        for i in range(len(index_vis)):
            ...
            vis2_flag = np.array(vis2_flag, dtype='bool')
        ...
        for i in range(len(index_t3)):
            ...
            t3_flag = np.array(t3_flag, dtype='bool')
        ...
        # -------------------------
        # TRAITEMENT DES FLUX (toujours DANS LE 'with', pour éviter file closed)
        # -------------------------
        for i in range(len(index_flux)):

            wavel_index = np.where(insame_wvl == fichier[index_flux[i]].header['INSNAME'])[0][0]

            # ---- Convertir systématiquement en 2D ----
            new_flux      = np.atleast_2d(fichier[index_flux[i]].data['FLUXDATA'])
            new_flux_err  = np.atleast_2d(fichier[index_flux[i]].data['FLUXERR'])
            new_flux_flag = np.atleast_2d(fichier[index_flux[i]].data['FLAG'])

            # Données de longueur d’onde (une ligne par configuration)
            new_wavel_flux = np.atleast_2d(
                np.repeat(fichier[index_wvl[wavel_index]].data['EFF_WAVE'], new_flux.shape[0])
            )
            ...
            if flux.size == 0:
                ...
                # Harmonisation du nombre de colonnes
        ...
        # Mise à jour du dictionnaire final (toujours ici, à l'intérieur)
        dic['FLUX'] = {}
    ...
    # fin du with -> fichier est fermé en toute sécurité
