...
path_data = "/home2/jdrevon/path/file.fits"
path_results_tmp = "/home2/jdrevon/rec/"
...
#                    fig6, axes6 = plt.subplots(num_rows, num_cols, figsize=(10, 5 * num_rows))
            ...
            for h in hyperparameter:
                ...
                q_MiRA, V2_MATISSE, V2_MATISSE_ERR, V2_MiRA, q_CP, CP_MiRA, CP_MATISSE = IMAGE_to_V2(DATA_OBS, path_results, 8) #compute the visibilities and closure phase

                
                image = orientation_image(path_results)
...
#                        axs6 = axes6[row, col] if num_rows > 1 else axes6[col]
                ...
                axes[k,0].imshow(image,extent=[min(x_image),max(x_image),min(x_image),max(x_image)], cmap = 'hot', origin='lower')#,norm=colors.PowerNorm(gamma=0.5))#, norm=colors.PowerNorm(gamma=1.5))
                ...
                axes[k,1].imshow(image_conv,extent=[min(x_image),max(x_image),min(x_image),max(x_image)], cmap = 'hot',  origin='lower')#,norm=colors.PowerNorm(gamma=0.5))#, norm=colors.PowerNorm(gamma=1.5))
